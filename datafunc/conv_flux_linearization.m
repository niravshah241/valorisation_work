...
%    only reasonable in decomp_mode==0, otherwise not returned.
%
...
if model.flux_linear
  ...
  if model.use_velocity_matrixfile==1
  ...
  else % model.use_velocity_matrixfile == 0
    %%%%%%%%%%% compute the fluxes instead of loading matrix from file  
    ...
    elseif isequal(model.name_flux, 'richards')
      ...
      [res1, res2] = inv_geo_trans_derivative(model,X,Y,...
                          {[1],[2],[1,1],[1,2]},{[1],[2],[2,1],[2,2]},2);
      d1  = res1{3} + res2{3};
      d2  = res1{4} + res2{4};
