...
%
% (To be used as base model for initial results of RBEvolOpt...)
...
model.set_time = @set_time_default; % not set_time!!!
...
model.orthonormalize= @model_orthonormalize_gram_schmidt; %new
...
model.RB_max_refinement_level = 5;
model.RB_element_indicator_mode='nodes_cogs_skippedrefs';
model.RB_element_indicator_s_max = 1;
model.RB_mixed_sensitivity_basis_generation =0;

model.get_inner_product_matrix = @(model_data)model_data.W;
model.inner_product_matrix_algorithm = @(model,model_data)model_data.W;
...
model.reduced_data_subset = @lin_evol_reduced_data_subset;%@(model, reduced_data)reduced_data; %evtl. replace?!?
