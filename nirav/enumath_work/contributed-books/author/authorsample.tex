...
\mathbb{V} = \lbrace \overrightarrow{\phi} \in (L^2(\mathcal{T}))^d | \ \overrightarrow{\phi} \in (P^D(\tau_k))^d \ , \ \forall \ {\tau_k} \in \mathcal{T} \rbrace \ .
\end{equation}
The space for pressure is 
\begin{equation} \label{pressure_test}
\mathbb{Q} = \lbrace \psi \in (L^2(\mathcal{T})) | \ \psi \in (P^{D-1}(\tau_k)) \ \forall \ {\tau_k} \in \mathcal{T} \rbrace \ .
\end{equation}
...
\draw (4,4) node[anchor=north]{${}$}
  ...
  -- (0,4) node[anchor=south]{${}$}
  -- cycle;
\draw[->] (2,2)--(2.5,2.5)node[label={[xshift=0cm, yshift=-0.7cm]${\overrightarrow{n}^+}$}]{};
\draw[->] (2,2)--(1.5,1.5)node[label={[xshift=0cm, yshift=0cm]${\overrightarrow{n}^-}$}]{};
\node at (1,1){$\mathcal{T^-}$};
\node at (3,3){$\mathcal{T^+}$};
\node at (1.7,3){$\overrightarrow{v}^+,p^+$};
\node at (2.3,1){$\overrightarrow{v}^-,p^-$};
\end{tikzpicture}
...
\begin{itemize}
\item Jump operator: 
$\left[\overrightarrow{\phi} \cdot \overrightarrow{n}\right] = \overrightarrow{\phi}^+ \cdot \overrightarrow{n}^+ + \overrightarrow{\phi}^- \cdot \overrightarrow{n}^-$ on $\Gamma$, $\left[\overrightarrow{\phi} \cdot \overrightarrow{n}\right] = \overrightarrow{\phi} \cdot \overrightarrow{n}$ on $\Gamma_D$.
...
Here, $A_{ij} = a_{IP}(\overrightarrow{\phi}_i,\overrightarrow{\phi}_j)$, $B_{ij} = b(\overrightarrow{\phi}_i,\psi_j) + (\lbrace \psi_j \rbrace , [n \cdot \phi_i])_{\Gamma \cup \Gamma_D}$, $F_1 = l_{IP}(\overrightarrow{\phi}_i)$ and $F_2 = (\psi_j,\overrightarrow{n} \cdot \overrightarrow{u}_D)_{\Gamma_D}$.

\textbf{TODO: ADD : Affine forms, Offline online decomposition, REVIEW : POD and Galerkin formulation, ADD : experimental results: Eigenvalue drop, error vs basis function, RB,DG Solution and error at one parameter value}
