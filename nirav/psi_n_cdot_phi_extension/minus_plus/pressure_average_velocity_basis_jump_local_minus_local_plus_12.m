function [res] = ...
    pressure_average_velocity_basis_jump_local_minus_local_plus_12...
...
% N is normal
q = ldg_evaluate_basis(llocal2local(grid,face_index_neighbour,llcoord),...
    paramsP);
% q is pressure basis
...
if tria_index_neighbour>0
    for i=1:1:paramsP.ndofs_per_element
        for j=1:1:params.ndofs_per_element
            res(i,j) = res(i,j) + 0.5  * q(i) * v(j,1) * N(2);
        end
    end
else
