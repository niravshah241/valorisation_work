function plot_data_sequence(varargin)
% function plot_data_sequence(model,[grid],data,params[, callbackfigure, cbhandle]) 
%
...
% slices. A new figure is opened. 
% If further parameters are set, the call is assumed to
% stem from a callback-function
%
% required fields in params:
%         plot_function: the name of the plot-function performing
%                        the plotting of a single slice, e.g.
%                        plot_element_data, plot_vertex_data
%
% optional field of params:
%    title:  string indicating the title of the newly opened figure
%    clim: 2-vector giving the range of the colormap. If this is
%          set, identical range is used for all slices. Default is 
%          the min and max of all slices.
%    clim_tight: if this flag is set, the colorbar is set tightly
%            to the range of every single data slice. Clearly only clim or 
%            clim_tight should be set.
%
...
% open figure
if nargin < 5 % no callback, open new figure
  ...
  slider = uicontrol(f,'Style','slider','Units','normalized',...
		    ...
		    'Callback','plot_data_sequence([],[],[],[],gcbf,gcbo);');  
...
if nargin >=5 % callback-function
%  disp('entered plot_data_sequence_callback...')
  cbf = varargin{5};
  cbo = varargin{6};
  ...
  replot(gcbf,gcbo,v);
%  disp('exiting plot_data_sequence_callback...')
end;
...
feval(ud.params.plot_function,ud.grid,d,ud.params);
